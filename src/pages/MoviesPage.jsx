...
function MoviesPage() {
  ...
  const fetchData = async () => {
    ...
    const url = value
      ? `https://api.themoviedb.org/3/search/movie?query=${value}&include_adult=true&language=en-US&page=${page}`
      : `https://api.themoviedb.org/3/trending/movie/day?language=en-US?api_key=${api}&page=${page}`;
    try {
  ...
  return (
    <>
      ...
      <Movies movies={movies} page={page} setPage={setPage} isMovie={true} />
    </>
  );
}
